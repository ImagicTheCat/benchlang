...
for _, captures in ipairs(results) do
  ...
  local f, err = io.open("results/"..table.concat(captures,"/",1,#captures-1)..
    "/"..impl..".data", "rb")
  if f then
    ...
    if hcfg and lcfg and ecfg and wcfg
        -- check versions
        and result.env_version == ecfg.version and result.work_version == wcfg.version
        -- check hash
        and result.impl_hash == impl_hash then
...
More informations on the https://github.com/benchlang/benchlang[project] page.
    ]])
    f:write("\n\n== Hosts\n\n")
...
do -- Write aggregated results per host/work.
      ...
      for step, args in ipairs(wcfg.steps) do -- each step
        ...
        for depth, s_f in ipairs(s_fs) do -- each depth file
          ...
          s_f:write("= ["..hcfg.title.."]({{site.baseurl}}/hosts/"..host..
            ") / ["..wcfg.title.."]({{site.baseurl}}/works/"..work..") results\n\n")
        ...
        for rank, entry in ipairs(step_results) do
          ...
          local err_str
          if measure.err then
            err_str = "err: "..(measure.err == "status" and
              measure.err.." "..measure.status or measure.err)
          else
            err_str = "OK"
          end
          for depth, s_f in ipairs(s_fs) do
            ...
            if display then
              local function format_time(time)
                return time and string.format("%."..FP.."f", time) or "--"
              end
              s_f:write(rank.." | ["..lcfg.title.."]({{site.baseurl}}/langs/"..r.lang..")"
                .." | ["..ecfg.title.."]({{site.baseurl}}/langs/"..r.lang.."/envs/"..r.env..")"
                .." | "..err_str
                .." | "..format_time(measure.min_time)
                .." | "..format_time(measure.min_utime)
                .." | "..format_time(measure.min_stime)
                .." | "..(measure.max_maxrss or "--")
                .." | ["..r.impl.."]({{site.github.repository_url}}/blob/master/langs/"..
                  r.lang.."/impls/"..work.."/"..r.impl..")\n")
            end
          end
        end
        for depth, s_f in ipairs(s_fs) do
